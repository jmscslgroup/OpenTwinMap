...
def visualizeWaysWithLidar(tdot_subset, cores=48):
    ...
    for wid, way_nodes in tqdm(tdot_subset.osm_handler.ways.items()):
        ...
        way_coordinates = tdot_subset.osm_handler.getWayCoordinates(
            wid, project_to_meters=True
        )
        way_bounding_box_meters = tdot_subset.osm_handler.getWayBoundingBox(
            wid, project_to_meters=True
        )
    ...
    ways_ribbons = list(
        tqdm(
            joblib.Parallel(return_as="generator", n_jobs=cores)(
                joblib.delayed(convertWaysToWaysRibbonsElevation)(
                    tdot_subset,
...
def computeLidarHeight(tdot_subset, pcd_points, way_coordinates, bridge):
    for i in range(len(bridge)):
        if bridge[i]:
            lidar_median_height = tdot_subset.lidarMedianHeightAtXYMeters(
...
def convertWaysToWaysRibbons(
    ...
    for i in range(len(way_coordinates)):
        way_current_height = tdot_subset.minHeightAtXYMeters(
    ...
    way_coordinates = computeLidarHeight(
        tdot_subset, pcd_points, way_coordinates, bridge
    )
    # print("Expanding.....")
    expanded = ExpandCenterlineToDenseRibbon(
        way_coordinates,
        lanes=lane_count,
...
def conformToDEMs(
    tdot_subset, way_coordinates, way_bounding_box_meters, lane_height=0.25
):
    dem_data = tdot_subset.loadDEMsFromBoundingBoxMeters(way_bounding_box_meters)
...
def processLidarCorrection(
    ...
    smoothed, way_coordinates = convertWaysToWaysRibbons(
        tdot_subset,
        pcd_points,
        way_coordinates,
        way_bounding_box_meters,
    ...
    if len(mse) > 0:
        ...
        osm_corrected_points = conformToDEMs(
            tdot_subset, np.asarray(adjusted_osm_points.points), way_bounding_box_meters
        )
        return (
...
def correctWaysWithLidar(tdot_subset, cores=32):
    ...
    way_correction_results = list(
        tqdm(
            joblib.Parallel(return_as="generator", n_jobs=cores)(
                joblib.delayed(processLidarCorrection)(
                    child_seed,
                    tdot_subset,
    ...
    for i in range(len(way_correction_results)):
        ...
        tdot_subset.osm_handler.annotateWayWithCorrectedPoints(
...
if __name__ == "__main__":
    ...
    tdot_subset.osm_handler.createCorrectedOSMFile(
        "SubsetSelection/osm_subset.osm",
        "SubsetSelection/osm_subset_corrected_all_lidar_for_elevation.osm",
    )
