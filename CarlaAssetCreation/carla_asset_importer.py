...
def _importFbxToUnrealMethodBase(
    ue4_root, project_path, dataset_path, asset_type, asset_names
):
    editor_cmd = os.path.join(ue4_root, "Engine/Binaries/Linux/UE4Editor-Cmd")
    script_name = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "./unreal_fbx_to_asset.py")
    )
...
def _importFbxToUnrealMethodGeneral(
    ue4_root, project_path, dataset_path, asset_type, asset_names, max_asset_amount=5000
):
    asset_names_chunked = [
        asset_names[i : i + max_asset_amount]
        for i in range(0, len(asset_names), max_asset_amount)
    ]
    for chunk in asset_names_chunked:
        _importFbxToUnrealMethodBase(
            ue4_root, project_path, dataset_path, asset_type, chunk
        )
...
class CarlaAssetImporter:
    ...
    def __init__(self, ue4_root, carla_path, cooked_path):
        ...
        self.project_path = os.path.join(
            carla_path, "Unreal/CarlaUE4/CarlaUE4.uproject"
        )
    ...
    def clearUnrealContent(self):
        ...
        script_name = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "./unreal_clear_assets.py")
        )
        args = [
            editor_cmd,
            self.project_path,
    ...
    def convertAssetTypeFromFbxToUnreal(self, asset_type, n_jobs=48):
        ...
        for i in range(n_jobs):
            asset_names_job_chunk = total_asset_names[
                (i * total_asset_names_chunk_size) : (
                    (i + 1) * total_asset_names_chunk_size
                )
            ]
            jobs.append(
                [
