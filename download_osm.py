...
def getBoundingBox(metadata_entry):
	try:
		...
		long_length = max_long - min_long
		lat_length = max_lat - min_lat
		long_extra = long_length
		lat_extra = lat_length

		return [min_long - long_extra, min_lat - lat_extra, max_long + long_extra, max_lat + lat_extra]
	except:
...
def download_osm(tile):
	...
	xml_osm_path = metadata_entry["OpenStreetMap"]["xml_path"]
	json_osm_path = metadata_entry["OpenStreetMap"]["json_path"]
	bounding_box = getBoundingBox(metadata_entry)
	print("Saving path ", xml_osm_path)
	print("Saving path ", json_osm_path)
	print("Bounding box ", bounding_box)
	#overpass --area 'Tennessee' --format json --bbox -86.6902731783606 36.0658924703269 -86.6800752272334 36.0714441392433 'node;way;'  > test3.josm
	command_string = "overpass --area 'Tennessee' --format xml --bbox {} {} {} {} 'node->.nodes; .nodes out meta; way->.outways; .outways out meta;' > {}".format(bounding_box[0], bounding_box[1], bounding_box[2], bounding_box[3], xml_osm_path)
	print("Invoking ", command_string)
	subprocess.run(command_string, shell=True)
	#command_string = "overpass --area 'Tennessee' --format json --bbox {} {} {} {} 'node->.nodes; .nodes out meta; way->.outways; .outways out meta;' > {}".format(bounding_box[0], bounding_box[1], bounding_box[2], bounding_box[3], json_osm_path)
	#print("Invoking ", command_string)
	#subprocess.run(command_string, shell=True)
